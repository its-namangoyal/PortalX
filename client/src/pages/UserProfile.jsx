...
import { apiRequest, handleFileUpload, handleResumeUpload } from "../utils";
...
const UserForm = ({ open, setOpen }) => {
  ...
  const onSubmit = async (data) => {
    ...
    // const resumeUri = uploadCv && (await handleResumeUpload(uploadCv));

    // Upload profile image if provided, otherwise keep the existing one
    const uri = profileImage ? await handleFileUpload(profileImage) : user.profileUrl;

    // Upload CV if provided, otherwise keep the existing one
    const resumeUri = uploadCv ? await handleResumeUpload(uploadCv) : user.cvUrl;

    // const newData = uri ? { ...data, profileUrl: uri, cvUrl: resumeUri } : data;
    const newData = { ...data, profileUrl: uri, cvUrl: resumeUri };
      ...
      <div className='w-full md:w-2/3 2xl:w-2/4 bg-white shadow-lg p-10 pb-20 rounded-lg'>
        <div className='flex flex-col items-center justify-center mb-4'>
          ...
          <h5 className='text-blue-700 text-base font-bold'>
            {userInfo?.projectTitle || "Add Project Title"}
          </h5>
        ...
        <div className='w-full py-10'>
          <div className='w-full flex flex-col-reverse md:flex-row gap-8 py-6'>
            ...
            <div className='w-full' onClick={handleThumbnailClick}>
              <Worker workerUrl={`https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.worker.min.js`}>
                { console.log("RESUME", userInfo?.uploadCv ) }
                <Viewer fileUrl={user?.cvUrl || NoProfile} /> 
              </Worker>
            </div>
